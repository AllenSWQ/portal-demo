 ...
 * 生成随机图片，用来作为验证码
 */
public class VerifiCodeFilter extends HttpServlet {
	private static Log log = LogFactory.getLog(VerifiCodeFilter.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < 40; i++) {  
            // 设置随机开始和结束坐标  
            int xs = randomV.nextInt(WIDTH);//x坐标开始  
            int ys = randomV.nextInt(HEIGHT);//y坐标开始  
            int xe = xs + randomV.nextInt(WIDTH / 30);//x坐标结束  
            int ye = ys + randomV.nextInt(HEIGHT / 30);//y坐标结束  
  
            // 产生随机的颜色值，让输出的每个干扰线的颜色值都将不同。  
            red = randomV.nextInt(255);  
            green = randomV.nextInt(255);  
            blue = randomV.nextInt(255);  
